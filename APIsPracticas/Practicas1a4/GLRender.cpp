...
void GLRender::SetupObject(Object3D * obj)
{
	...
	for (int mesh = 0; mesh < meshCount; ++mesh) {
		...
		glBufferData(GL_ARRAY_BUFFER, vertexSize * sizeof(float) * sizeof(Vertex), objMesh->GetVertices().data(), GL_STATIC_DRAW);


		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, meshAttributes.indexArrayID);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(int) * vertexIDsSize * 3, objMesh->GetVertexIdxList().data(), GL_STATIC_DRAW);
